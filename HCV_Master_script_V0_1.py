...
def main():
        ...
        for sample in sample_order:
            ...
            if sample in sample_snp_distances and sample_snp_distances[sample]:
                ...
                for linked_sample, distance, shared_haplotype_percent, shared_count, _ in sorted_distances: # Ignore total_count_for_sample here
                    ...
                    shared_haplotype_percent = shared_count/total_haplotypes_in_sample * 100 if shared_count > 0 else 0
    ...
    # --- Final Cleanup of Sample Temporary Directories ---
    print("\n--- Running Final Cleanup ---", file=sys.stderr)
    if not args.keep_tmp:
        print("Removing temporary directories for each sample...", file=sys.stderr)
        # The sample_temp_dirs dictionary was populated in Step 1
        for prefix, temp_dir_to_remove in sample_temp_dirs.items():
            if temp_dir_to_remove and temp_dir_to_remove.exists():
                print(f"  Removing temporary directory for sample {prefix}: {temp_dir_to_remove}", file=sys.stderr)
                shutil.rmtree(temp_dir_to_remove, ignore_errors=True)
            elif temp_dir_to_remove and temp_dir_to_remove.exists() and args.keep_tmp: # This condition is actually redundant due to the outer if
                print(f"  Keeping temporary directory for sample {prefix}: {temp_dir_to_remove}", file=sys.stderr)
    elif args.keep_tmp:
        print("Keeping temporary directories for each sample as per --keep_tmp flag.", file=sys.stderr)
        for prefix, temp_dir_to_keep in sample_temp_dirs.items():
            if temp_dir_to_keep and temp_dir_to_keep.exists():
                 print(f"  Keeping temporary directory for sample {prefix}: {temp_dir_to_keep}", file=sys.stderr)
