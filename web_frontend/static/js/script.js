document.addEventListener('DOMContentLoaded', function() {
    ...
    const textSortBy = document.getElementById('text-sort-by');
    const textSortOrder = document.getElementById('text-sort-order');
    const htmlSortBy = document.getElementById('html-sort-by');
    const htmlSortOrder = document.getElementById('html-sort-order');
    ...
    function fetchReports() {
        const textSortParams = `sort_by=${textSortBy.value}&order=${textSortOrder.value}`;
        const htmlSortParams = `sort_by=${htmlSortBy.value}&order=${htmlSortOrder.value}`;
        
        fetch(`/list_files?${textSortParams}&${htmlSortParams}`)
            .then(response => response.json())
            .then(data => {
                allTextFiles = data.text_files;
                allHtmlFiles = data.html_files;

                renderFiles(allTextFiles, textReportsList, textReportsPagination, currentTextPage, 'text');
                renderFiles(allHtmlFiles, htmlReportsList, htmlReportsPagination, currentHtmlPage, 'html');
            })
            .catch(error => {
    ...
    textSortBy.addEventListener('change', function() {
        currentTextPage = 1;
        fetchReports();
    });
    
    textSortOrder.addEventListener('change', function() {
        currentTextPage = 1;
        fetchReports();
    });
    
    htmlSortBy.addEventListener('change', function() {
        currentHtmlPage = 1;
        fetchReports();
    });
    
    htmlSortOrder.addEventListener('change', function() {
        currentHtmlPage = 1;
        fetchReports();
    });
