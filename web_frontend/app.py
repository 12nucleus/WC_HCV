...
def list_files():
    sort_by = request.args.get('sort_by', 'name')  # 'name' or 'date'
    ...
    for f in REPORTS_DIR.glob('*.txt'):
        report_files.append({
            'path': str(f.relative_to(PIPELINE_BASE_DIR)),
            'name': f.name,
            'date': f.stat().st_mtime
        })
    for f in REPORTS_DIR.glob('*.out'):
        report_files.append({
            'path': str(f.relative_to(PIPELINE_BASE_DIR)),
            'name': f.name,
            'date': f.stat().st_mtime
    ...
    sorted_report_files = sorted(report_files, key=get_sort_key, reverse=reverse_order)
    sorted_html_files = sorted(html_files, key=get_sort_key, reverse=reverse_order)

    return jsonify({
        'text_files': [f['path'] for f in sorted_report_files],
        'html_files': [f['path'] for f in sorted_html_files],
        'sort_by': sort_by,
        'order': order
    })
